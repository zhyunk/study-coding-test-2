...
/**
https://school.programmers.co.kr/learn/courses/30/lessons/181858?language=kotlin
 *문제 설명
...
class Solution195 {
    fun solution(arr: IntArray, k: Int): IntArray {
        ...
        return IntArray(k) {
            if (it < distinctArr.size) distinctArr[it] else -1
        }
    }

    /*
    fun solution(arr: IntArray, k: Int): IntArray {
        val answer = IntArray(k) { -1 }
        var i = 0

        arr.forEach {
            if (i == k) return answer
            if (!answer.contains(it)) answer[i++] = it
        }
...
fun main() {
    println(Solution195().solution(intArrayOf(0, 1, 1, 2, 2, 3), 3).joinToString(", "))
    println(Solution195().solution(intArrayOf(0, 1, 1, 1, 1), 4).joinToString(", "))
}
