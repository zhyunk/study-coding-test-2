...
/**
https://school.programmers.co.kr/learn/courses/30/lessons/181881?language=kotlin
 *문제 설명
 * 정수 배열 arr가 주어집니다.
...
class Solution152 {
    fun solution(arr: IntArray): Int {
        var i = 0
        var cpArr = arr.copyOf()

        while (true) {
            if (cpArr.contentEquals(next(cpArr))) return i
    ...
    fun next(arr: IntArray): IntArray {
        return arr.map { when {
            it >= 50 && it % 2 == 0 -> it / 2
            it < 50 && it % 2 == 1 -> it * 2 + 1
            else -> it
...
fun main() {
    println(Solution152().solution(intArrayOf(1, 2, 3, 100, 99, 98)))
}
