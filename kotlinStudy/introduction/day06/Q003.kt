...
/**
https://school.programmers.co.kr/learn/courses/30/lessons/181925?language=kotlin
 *문제 설명
 * 정수 배열 numLog가 주어집니다.
...
class Solution63 {
    ...
    fun solution(numLog: IntArray): String {
        return (1..numLog.lastIndex).joinToString("") {
    ...
    fun solution(numLog: IntArray): String {
        ...
        return numLog.foldIndexed(""){ index, acc, i -> acc + if (index == numLog.lastIndex) "" else map.getValue(numLog[index+1] - i) }
    }
    */
    ...
    fun solution(numLog: IntArray): String {
        val map = mapOf(1 to 'w', -1 to 's', 10 to 'd', -10 to 'a')
        var res = ""
        for (i in 0 until numLog.lastIndex) {
            res += map.getValue(numLog[i+1]-numLog[i])
        }
        return res
    }
*/
...
fun main() {
    println(Solution63().solution(intArrayOf(0, 1, 0, 10, 0, 1, 0, 10, 0, -1, -2, -1)))
}
