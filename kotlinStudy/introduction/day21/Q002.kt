...
/**
https://school.programmers.co.kr/learn/courses/30/lessons/181851?language=kotlin
 *문제 설명
...
class Solution212 {
    fun solution(rank: IntArray, attendance: BooleanArray): Int {
        return rank.mapIndexed { index, i -> Pair(i, index) }
            .filter { i -> attendance[i.second] }
            .sortedBy { it.first }
            .take(3)
            .let { (a, b, c) -> (10000 * a.second) + (100 * b.second) + c.second }
    }

}

fun main() {
    println(Solution212().solution(intArrayOf(3, 7, 2, 5, 4, 6, 1), booleanArrayOf(false, true, true, true, true, false, false)))
}
