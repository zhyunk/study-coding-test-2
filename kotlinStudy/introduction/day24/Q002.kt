package kotlinStudy.introduction.day24


/**
https://school.programmers.co.kr/learn/courses/30/lessons/181836?language=kotlin
 *문제 설명
...
class Solution242 {
    fun solution(picture: Array<String>, k: Int): Array<String> {
        return picture.fold(arrayOf<String>()) { acc, i ->
            val s = i.toCharArray().joinToString("") { j -> "$j".repeat(k) }

            acc + Array<String>(k) { s }
        }
    }
...
fun main() {
    println(Solution242().solution(arrayOf(".xx...xx.", "x..x.x..x", "x...x...x", ".x.....x.", "..x...x..", "...x.x...", "....x...."), 2).joinToString(", "))
    println(Solution242().solution(arrayOf("x.x", ".x.", "x.x"), 3).joinToString(", "))
}
